import pytest
from schemas.image_info import (
  ImageInfo,
  ImageStatus,
  IMAGE_INFO_SCHEMA,
)
...
def test_image_info_default_status():
  ...
  image_info = ImageInfo(filename="test_image.jpg", date=1, theme="wildlife", real=True)
...
def test_image_info_schema_load_default_status():
  ...
  data = {"filename": "test_image.jpg", "date": 1, "theme": "wildlife", "real": True}
  result = IMAGE_INFO_SCHEMA.load(data)
  assert result.filename == "test_image.jpg"
  assert result.date == 1
  assert result.theme == "wildlife"
